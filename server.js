...
app.post("/purchases", async (req, res) => {
  ...
  try {
    ...
    for (const item of items) {
      ...
      await db.query(
        "INSERT INTO purchases (item_name, supplier_id, purchase_date, quantity, unit_price, total_cost, expiry_date, bill_no, brand, units, domain, category_name, description) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
        [
          item_name,
          supplier_id,
          purchase_date,
      ...
      try {
        await db.query(
          "INSERT INTO items (name, category_name, brand, supplier_id, quantity, units, unit_price, description, domain, expiry_date) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
          [
            item_name,
            category_name,
            brand,
...
app.get("/purchaselist", async (req, res) => {
  ...
  if (bill_no) {
    query += ` AND b.bill_no LIKE ?`; // Apply the filter to the bill table
    params.push(`%${bill_no}%`);
    console.log("Bill No Filter:", bill_no); // Debugging log
  }

  try {
    const [data] = await db.query(query, params);
    console.log("Query Results:", data); // Debugging log 
    console.log("Invoice Data:", data.map(row => row.invoice));
    data.forEach((row) => {
      if (row.invoice) {
